...
const leftWidth = computed(() => {
    return convertToCss(props.leftMinWidth) || convertToCss(props.leftMaxWidth) || 'auto'
})
...
<template>
    ...
    <div class="cc-horizontal" :class="{ 'full-height': fullHeight }" :style="containerStyle">
        <!-- Left Sidebar -->
        <div class="c-left">
            <slot name="left"></slot>
        </div>
        <div class="cc-main" :class="{  'center-content': centerContent, 'vertical-center': verticalCenterContent }" :style="{ minHeight: minHeight }">
            <div :style="{ maxWidth: contentMaxWidth, width: contentWidth ? contentWidth : null }">
                <slot></slot>
            </div>
        </div>
        <!-- Right Section -->
        <div class="c-right" :style="{ minWidth: rightMinWidth, maxWidth: rightMaxWidth }">
...
.cc-main.vertical-center, .c-right,.c-left {
    align-items: center;
}
.cc-main > div {
    /* height: 100%; */
